...
function sim_FUNCinvnfault_MCv2(invf,noloops)
%
%************** FWP Work ************************
...
global input fpara Inv index locals alpha symbols weighs cinput am wmatrix mvcm wVCM ...
       rake_value rake_isinv inv_VCM rakeinfo initialONE UTMZONE mwconinfo mwall obsunit
%
iscov = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[psoPS,cinput,symbols,outoksar,abccof,outmatf,fpara,...
 lamda,myu,scamin,scamax,Inv,locals,weighs,vcms,...
 ntimes,iterations,display,simiters,ismc,mcloops,...
 mcdir,mcsave,fismc,disofparts,rake_value,rake_isinv,isvcm,rakeinfo,mwconinfo,...
...
% Update utmzone from inp file finally... 
...
% cyeind, the indices of the data set which will be inverted...
% cnoind, the not 
%
[am,cyeind,cnoind] = sim_mergin(cinput,abccof);
%
...
for ni=1:ndata
    ...
    if isvcm == 1
        if strcmpi(vcms{cindex(ni)},'NULL')==0
            ...
            % @ GU, 06/10/2011
            %
            [~,bname] = fileparts(vcms{cindex(ni)}{1});
            [~,~,post]= fileparts(bname);
            %
...
% if nargin < 5 || isempty(simiters)==1
%    simiters = 10000000;
% end
...
for ni=1:ntimes
    ...
    % fixed a bug by Feng, W.P., 20110819, @ BJ
    %
    if exist(savedir,'dir')==0
        mkdir(savedir);
    end
    ...
    if strcmpi(fext,'.simp')
       sim_fpara2simp(sfpara,fullfile(savedir,['MC_' num2str(noloops) '_' coutoksar]),UTMZONE);
    else
       sim_fpara2oksar_SYN(sfpara,fullfile(savedir,['MC_' num2str(noloops) '_' coutoksar]),UTMZONE);
    end
    %
    oksar2gmt(fullfile(savedir,['MC_' num2str(noloops) '_' coutoksar]),[outname,'_'],'stitle',['MC_' num2str(noloops) '_' coutoksar],'outindex',4,'utmzone',UTMZONE,'minstd',min([vobj1,vobj2]));
    sim_okinv2res(sfpara,mabc(:),ni,alpha);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    eval(['save ' fullfile(savedir,['MC_' num2str(noloops) '_' outmatf{1}{1}]) ' outfpara outfpswm outstd fpara abcm abcsm cinput cyeind cnoind']);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    disp([fullfile(savedir,['MC_' num2str(noloops) '_' outmatf{1}{1}]) ' has been saved!']);
    disp(['Congratulations! PSOKINV finished JOB Number ' num2str(ni) ';' ' MC inversion number: ' num2str(noloops) '.']);
end
nowtime = datestr(now());
