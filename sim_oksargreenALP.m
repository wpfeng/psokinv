function [green,sgreen,dgreen,tgreen] = sim_oksargreenALP(fpara,input,thd,alp,rakes,meanstrike,meandip,meandepth)
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global earthmodel utmzone globalinfo isokada3d
%
isoutput = 0;
%
if nargin < 6
...
if isokada3d >= 1
    if isokada3d == 1
       disp(' ***Okada3D*** with rectangle sources');
    else
       disp(' ***Okada3D*** with point-like sources'); 
    end
    if numel(input(1,:))==7
       disp(' ***Okada3D*** Z is fixed at depth 0km.'); 
       z = input(:,1).*0;
    else
       z = input(:,8);
    end
...
tgreen = sgreen;
%
% disp([' GREENs Function: ',num2str(isokada3d)]);
for nd = 1:ndim
    %
    ...
    % Updated by FWP,      @UoG, 2013-05-14
    %
    if strcmpi(earthmodel,'ELA')
        %
        % Updated by Wanpeng Feng, @NRCan, 2016-04-21
        % Surface amplitude variation is taken into account...
        %
        if isokada3d == 0
           dis = multiokadaALP(fpara(nd,:),x,y,0,alp,thd);
        else
           if isokada3d == 1
              dis = multiokada3D(fpara(nd,:),x,y,z,0,alp,thd,0);
           else
              dis = multiokada3D(fpara(nd,:),x,y,z,0,alp,thd,1);
           end
        end
    else
        %
    ...
    if nargout == 4
        ...
        % 2013-05-22
        mdata = [];
        disp(['PS_SMEST: Making the Green Matrix (Opening) by PSCMP. Now working on NO: ',num2str(nd,'%5.0f'),' fault...']);
        %
           for pscmpni = 1:nfile
              %
              cfpara = fpara(nd,:);
