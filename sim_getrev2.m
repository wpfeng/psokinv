function sim_getrev2(dire,postfix,outdir,prefix,isll)
%
%************** FWP Work ************************
...
% Modified by W. Feng, @SYSU, Guangzhou, 2022/11/17
%    - change the default fault format as simp
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin<1 || isempty(dire)==1
   ...
   disp('       postfix,  the postfix for searching, default /*.simp/');
   disp('         outdir, the folder to save results,default /./');
   disp(['Example: sim_getrev2(' '''' '.' '''' ',' '''' '*.simp*' '''' ');']);
   disp('-------------------------------------------------------------------');
   return
end
%
...
for nj=1:numel(file)
    ...
    %
    % updated by FWP, @SYSU, Guangzhou, 2022/11/17
