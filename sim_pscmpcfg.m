function [topdir,outfilenames] = sim_pscmpcfg(fpara,obsp,varargin)
%
%
% +Purpose:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
[topdir,outdir] = fileparts(outinp);
if isempty(topdir)
    topdir = pwd;
end
%
if ~exist([topdir,'/',outdir],'dir')
    mkdir([topdir,'/',outdir]);
end
fprintf(fid,'%s\n',['''',topdir,'/',outdir,'/','''']);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf(fid,'%6.0f\n',numel(days));
%
outfilenames = cell(numel(days),1);
fidtime = fopen([topdir,'/',outdir,'/pscmp.time'],'w');
%
for ni = 1:numel(days)
    outfilenames{ni} = ['def_no',num2str(days(ni)),'.dat'];
    fprintf(fid,'%6.0f %s\n',days(ni),['   ','''',outfilenames{ni},'''']);
    fprintf(fidtime,'%f %s\n',days(ni),outfilenames{ni});
end
fclose(fidtime);
%
...
if ischar(fpara)
    %
    [fpara,utmproj] = sim_openfault(fpara);
    %
end
%
%
% To get more accurate calculation, we may need to oversample each fault
% patch with large size if required...
%
%
if isove == 1
...
for ni = 1:numel(fpara(:,1))
    ...
    fprintf(fid,'%d %f %f %f %f %f %f %f %d %d %d\n',[ni,clat,clon,tfpara(5),...
           tfpara(7),tfpara(6),tfpara(3),tfpara(4),np_st,np_di,0] );%    0.00');
